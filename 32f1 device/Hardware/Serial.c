...
void USART1_IRQHandler(void)
{
	...
	{
		uint8_t RxData1 = USART_ReceiveData(USART1);				//读取数据寄存器，存放在接收的数据变量
...
void USART3_IRQHandler(void)
{
	...
	{
		uint8_t RxData3 = USART_ReceiveData(USART3);				//读取数据寄存器，存放在接收的数据变量
		...
		{
			Serial_RxPacket2[pRxPacket] = RxData3;	//将数据存入数据包数组的指定位置
			pRxPacket ++;				//数据包的位置自增
